...
#define list_graph_file_name             "graph"
#define list_graph_describtion_file_name "work/graph_describtion"

const double list_resize_coefficient = 2;
...
\
    list_generate_graph_describtion (x);\
    list_generate_graph ();\
    _flist_graphdump (x, list_graph_dump_file_name, __FILE__, __PRETTY_FUNCTION__, __LINE__, __VA_ARGS__)
...
Element      list_pop_back      (List* list, Return_code* return_code_ptr = nullptr, bool shrink = false);
Element      list_pop_front     (List* list, Return_code* return_code_ptr = nullptr, bool shrink = false);
...
int list_search_physical_index_given_logical_index (List* list, int logical_num);
